...
export class DatabaseSDK {
  ...
  async getRecords<T>(
    ...
    const url = `${this.baseUrl}/records/${tableName}${
  ...
  async createRecord<T extends Record<string, any>>(
    ...
    const url = `${this.baseUrl}/data/${tableName}`;
  ...
  async updateRecord<T extends Record<string, any>>(
    ...
    const url = `${this.baseUrl}/data/${tableName}/${id}`;
  ...
  private validateTable(tableName: string): void {
    if (!this.validTables.includes(tableName)) {
      throw new Error(
        `Invalid table name. Must be one of: ${this.validTables.join(", ")}`
      );
    }
  }
